...
const FilterSiderBar = () => {
  ...
  const handleFilterChange = (e:any)=>{
    ...
    if(type === 'checkbox'){
      if(checked){
        newFitlers[name] = [...(newFitlers[name] || []),value]
      } else{
        newFitlers[name] = newFitlers[name].filter((item)=>item !== value)
      }
    }else{
      newFitlers[name] = value
    }

    setFilters(newFitlers)
    updateURlParams(newFitlers)
  }

  const updateURlParams = (newFilter: any)=>{
    const params = new URLSearchParams()
    Object.keys(newFilter).forEach((item)=>{
      if(Array.isArray(newFilter[item]) && newFilter[item].length>0){
        params.append(item, newFilter[item].join(','))
      }else if(newFilter[item]){
        params.append(item, newFilter[item])
      }
    })
